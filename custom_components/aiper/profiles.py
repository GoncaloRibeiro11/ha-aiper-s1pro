...
def device_family(device: dict[str, Any]) -> DeviceFamily:
    ...
    candidate_fields = [
        device_model_string(device).lower(),
        ...
        str(device.get("productName") or "").lower(),
        str(device.get("productModel") or "").lower(),
    ]
    candidates = " ".join(candidate_fields)
    normalized_candidates = candidates.replace("-", " ").replace("_", " ")

    if any(
        marker in candidates or marker in normalized_candidates
        for marker in SCUBA_MODEL_MARKERS
    ):
        return DeviceFamily.SCUBA
    if any(marker in candidates for marker in SURFER_MODEL_MARKERS):
        return DeviceFamily.SURFER
    if any(marker in candidates for marker in SHARK_MODEL_MARKERS):
